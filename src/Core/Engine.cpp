...
// -----------------------------------------------------------------------------------------------------------------
...

    {
        ...
        if (loader.albedo.has_value()) {
            texture1.stream_texture_from_data(loader.albedo.value(), "Avocade texture");
            view1 = texture1.load_texture(renderer.loop_task_graph);
        }

        meshManager.add_mesh("Avocado", loader.getModelData(0).vertexCount, loader.getModelData(0).indexCount);
     ...
     // meshManager.add_mesh("Cube", 8, 36);
     ...
     // meshManager.add_mesh("Cube 2", 24 ,36);
     ...
     // meshManager.add_mesh("Cube3", 8, 36);
          ...
          for (int x = 0; x < grid_size; ++x) {
              for (int y = 0; y < grid_size; ++y) {
                  for (int z = 0; z < grid_size; ++z) {
                      ...
                      data.model_matrix = glm::scale(glm::translate(glm::mat4(1.0f), position), glm::vec3(25.0f));
                      data.texture = view1;
        ...
        for (int x = 0; x < grid_size; ++x) {
            for (int y = 0; y < grid_size; ++y) {
                for (int z = 0; z < grid_size; ++z) {
                    ...
                    drawGroup.meshes[0].lock()->instance_data[cubeIndex].model_matrix = glm::scale(glm::translate(glm::mat4(1.0f), position) * glm::rotate(glm::mat4(1.0f), angle, glm::vec3(0, 1, 0)), glm::vec3(25.0f));
                }
            }
        }
        auto* ptr = device.buffer_host_address_as<PerInstanceData>(drawGroup.meshes[0].lock()->instance_buffer_id).value();
